...
from ..core.pipeline import run_gouvernance_sequence
from ..modules import accounting, biffageops, docai, docuscope, factureops, privacyops
...
def run_light_pipeline(instance: InstanceConfig, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if _depot_porte_une_assemblee(instance, manifest):
        # La suite est definie une seule fois, dans `core.pipeline`: elle etait
        # ecrite ici et absente de la chaine CLI, et les deux copies divergeaient.
        run_gouvernance_sequence(
            instance,
            run,
            step=lambda nom, runner: _run_step(instance, manifest, nom, runner),
        )
    run.finish("OK" if manifest.get("status") != "error" else "WARN", f"deposit={manifest['deposit_id']}")
    return manifest


def _depot_porte_une_assemblee(instance: InstanceConfig, manifest: dict[str, Any]) -> bool:
    """Ce depot porte-t-il de la matiere d'assemblee a traiter.

    Le traitement reagit a ce qui est depose plutot que de tout relancer: une
    piece d'assemblee deposee doit produire ses resolutions sans que
    l'utilisateur ait a declencher une seconde chaine dont rien ne lui signale
    l'existence.

    La question porte sur l'AXE `ce depot releve-t-il de l'assemblee`, pas sur
    une etiquette. L'ancienne garde exigeait `PV_AG` exactement: un cabinet qui
    type `Proces_Verbal_AG` ne declenchait rien et l'ecran restait muet. Les
    etapes appelees se degradant proprement, sur-declencher ne coute qu'un
    passage a vide la ou sous-declencher coute un ecran vide sans raison.
    """
    from ..core.common import read_csv

    wanted = set(_doc_ids_for_deposit(instance, manifest))
    if not wanted:
        return False
    _, docs = read_csv(instance.register("documents"))
    return any(
        doc.get("doc_id") in wanted and _releve_de_l_assemblee(doc.get("document_type", ""))
        for doc in docs
    )


def _releve_de_l_assemblee(document_type: str) -> bool:
    """Compare sur des JETONS, pas sur une egalite de chaine.

    `PV_AG`, `Proces_Verbal_AG`, `Convocation_AG` et `Annexe_AG` relevent tous
    de l'assemblee. Un type inconnu ne declenche rien: degradation voulue.
    """
    jetons = {jeton for jeton in re.split(r"[^A-Za-z]+", document_type.upper()) if jeton}
    return bool(jetons & {"AG", "ASSEMBLEE"})
