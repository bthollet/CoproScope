...
from .compta_rapprochement_provenance import (  # noqa: F401
    est_marqueur_vide,
    premiere_valeur,
    premiere_valeur_et_source,
    signal_montant,
)
...
def build_compta_reconciliation_view(
    ...
    return {
        ...
        "shell_model": _shell_model(year, instance),
    }
...
def _cells(row: Mapping[str, Any], priority: str) -> list[dict[str, str]]:
    ...
    montant_brut, champ_montant = premiere_valeur_et_source(row, "ttc", "montant_depense", "amount")
    invoice_amount = _money(montant_brut)
    ...
    return [
        ...
        _cell(
            ...
            primary_signal=signal_montant(invoice_amount, champ_montant),
        ),
        _cell(
            "Decision / devis",
            decision_status,
...
_first = premiere_valeur


_is_empty_marker = est_marqueur_vide
