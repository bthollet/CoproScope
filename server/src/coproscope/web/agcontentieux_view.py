...
def _question_from_ag_row(row: dict[str, str], ag_id: str, index: int, year: int) -> dict[str, Any]:
    ...
    return {
        ...
        # **Ni ici non plus.** Le registre d'assemblee porte sa propre colonne
        # `file_name`, et c'est par elle que le nom brut continuait de sortir
        # apres la correction de la carte de document. Une piece se designe par
        # son identifiant public; le libelle derive dit ce qu'elle est.
        "source_ref": row.get("doc_id") or "",
        "proof_ref": libelle_public_document(row) if row.get("doc_id") else "",
        "restriction": RESTRICTION_RESTRICTED,
        "restriction_label": _restriction_label(RESTRICTION_RESTRICTED),
        "diffusion": DIFFUSION_CS,
        "diffusion_label": _diffusion_label(DIFFUSION_CS),
...
def _document_card(row: dict[str, str]) -> dict[str, Any]:
    ...
    return {
        ...
        # **Ni le nom brut, ni l'identifiant.** Mesure du 2026-09-08: cet ecran
        # rendait `file_name` en titre et retombait sur `doc_id` - donc le nom
        # brut d'un cote (ce que le lot biffage a retire ailleurs) et un
        # identifiant technique en face de l'utilisateur de l'autre, ce que
        # `RM-2026-0052` reproche depuis le 2026-09-03.
        #
        # On reutilise le libelle derive que le produit possede deja plutot que
        # d'en ecrire un second: deux facons de nommer une meme piece est le
        # defaut numero un de ce produit, et il s'installe exactement comme ca.
        "title": libelle_public_document(row),
