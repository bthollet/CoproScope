...
from .fiche_document_public import libelle_de_fiche, pseudonymise_identites
...
def build_document_detail(instance: InstanceConfig, doc_id: str, page: str = "") -> dict[str, object]:
    ...
    row = pseudonymise_identites(instance, row)
    ...
    return {
        ...
        "diffusion": diffusion,
        "signature": signature,
        "pdf_trace": _pdf_trace(instance, row, preview, page),
...
def _public_document(row: dict[str, str]) -> dict[str, str]:
    return {
        ...
        "file_name": libelle_de_fiche(row),
...
def _pdf_trace(instance: InstanceConfig, row: dict[str, str], preview: dict[str, object],
               page: str = "") -> dict[str, object]:
    ...
    # `RM-2026-0157`. Le lecteur s'ouvrait toujours page 1, meme suivi depuis
    # une citation qui nomme une page. Une demande hors bornes est ramenee a la
    # page 1 EN LE DISANT: se rabattre en silence ferait lire la mauvaise page
    # en croyant lire la bonne.
    page_ouverte, page_ecart = page_d_ouverture(page, page_count)
    return {
        "available": is_pdf,
        ...
        "current_page": page_ouverte,
        "page_ecart": page_ecart,
