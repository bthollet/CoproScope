...
def _find_piece_item(model: dict[str, object], piece_id: str) -> dict[str, object]:
    ...
    for item in candidates:
        ...
        merged.update(item)
        return merged
    # **LA BRANCHE DE REPLI EST RETIREE ICI, et la mesure l'a exigee.**
    # Elle rendait une piece ENTIEREMENT INVENTEE - libelle, criticite
    # `Critique`, motif redige, boutons de demande et de depot - des que
    # l'identifiant `UX-PIECE-COMP-C2B3F479` etait demande sans que le modele
    # le porte. Elle la marquait `is_fictive: True`, et **ce drapeau
    # n'atteignait AUCUN gabarit**: pose trois fois dans ce module, rendu zero
    # fois dans `web/templates/`. Le code savait que la piece etait fausse et
    # ne le disait nulle part.
    #
    # **L'item `RM-2026-0109` declarait honnetement n'avoir pas reussi a la
    # faire servir. La mesure du 2026-09-11 leve cette reserve:** sur un
    # modele VIDE - c'est-a-dire toute instance dont le coffre est absent ou
    # vide - `build_piece_detail` rendait `state: found` avec `is_fictive:
    # True`, tandis qu'un identifiant quelconque rendait `missing`. Le repli
    # n'etait donc pas mort: il etait atteignable par le cas le plus COURANT
    # d'une instance mal configuree.
    #
    # **La regle appliquee est celle de Brice, et elle renverse le diagnostic
    # d'origine:** *une instance sans coffre doit afficher ZERO octet, parce
    # que l'outillage est independant de l'instance*. Ce qui etait en cause
    # n'etait pas la pauvrete du chemin reel, c'est que le chemin sans coffre
    # affiche quoi que ce soit - et ici il affichait une piece inventee
    # presentee comme trouvee.
    #
    # Un identifiant que le modele ne porte pas retombe desormais sur le meme
    # `missing` que n'importe quel autre. **Aucun cas particulier ne survit**,
    # et c'est le point: un identifiant privilegie dans le code est une
    # modalite, pas un axe.
    return {}
...
def _empty_detail(piece_id: str) -> dict[str, object]:
    ...
    return {
        ...
        "piece": {
            "id": safe_id,
            # **`coffre local fictif` affirmait ce qu'il ne savait pas.** Ce
            # libelle sort sur TOUTE instance dont le modele ne porte pas
            # l'identifiant demande - y compris une instance reelle, dont le
            # coffre n'a rien de fictif. C'est le meme defaut que `Devis
            # retenu` (`RM-2026-0164`): un mot qui presuppose. Ce que l'ecran
            # sait, c'est que la reference n'est pas suivie ici; il ne sait
            # rien de la nature du coffre.
            "label": "Pièce introuvable dans les données chargées",
