...
def _accounting_total_charges(
    ...
    """Le total des charges, **avec la source qui l'a produit**. Lot `RM-2026-0075`.

    La cascade des sources est conservee: un total de grand livre reconstitue
    reste une information utile. Ce qui change est qu'elle ne franchit plus la
    frontiere sans son etiquette. L'appelant recoit de quoi decider s'il a le
    droit de l'afficher sous le libelle `Total charges`, et la reponse est non
    des que la source effective n'est pas l'etat des depenses.
    """

    total, count = _accounting_sum_unique(
        expense_statement.rows,
        amount_fields=("amount", "statement_amount", "ttc"),
        source=SOURCE_ETAT_DEPENSES,
    )
    if count:
        return MontantLu(valeur=total, source=SOURCE_ETAT_DEPENSES, lignes=count)
    total, count = _accounting_sum_unique(
        ledger.rows,
        amount_fields=("debit", "amount", "statement_amount", "ttc"),
        source=SOURCE_GRAND_LIVRE,
    )
    if count:
        return MontantLu(valeur=total, source=SOURCE_GRAND_LIVRE, lignes=count)
    fallback_rows = matches.rows + non_matches.rows + invoices.rows
    total, count = _accounting_sum_unique(
        fallback_rows,
        amount_fields=("statement_amount", "ttc", "montant_depense", "amount"),
        source=SOURCE_FACTURES,
    )
    if count:
        return MontantLu(valeur=total, source=SOURCE_FACTURES, lignes=count)
    return MontantLu()
