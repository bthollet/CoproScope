...
from ._montant_source import NON_CALCULABLE
...
def _document_display_label(row: dict[str, str]) -> str:
    """Le libelle d'un document, derive de ce qu'il EST.

    Cette fonction lisait `file_name` en premier, et affichait donc en clair des
    noms de fichiers ecrits par le syndic - dont, le 2026-09-07, un patronyme
    reel sur la page qui decide de la diffusion. Le garde `_safe_basename` ne
    bloquait que les chemins, jamais un nom de fichier nu. Voir la doctrine
    complete dans `_libelle_public.py`.
    """
...
#: Champs retires des lignes exposees aux gabarits. Les garder et se souvenir de
#: ne pas les afficher est une discipline; les retirer est une garantie. Le
#: 2026-09-07, `workstreams.html:449` lisait `row.file_name` en direct, sans
#: passer par aucun garde - c'est ce genre d'oubli que cette liste rend
#: impossible.
_CHAMPS_NON_EXPOSES = ("file_name", "filename", "original_path", "text_path", "redacted_path")
...
def _safe_privacy_rows(table: DataTable) -> DataTable:
    ...
    for row in table.rows:
        safe = dict(row)
        safe["display_file_name"] = _document_display_label(row)
        for champ in _CHAMPS_NON_EXPOSES:
            safe.pop(champ, None)
        safe["display_doc_id"] = _public_doc_id(row.get("doc_id", ""))
...
def _accounting_model(instance: InstanceConfig, year: int) -> dict[str, object]:
    ...
    # `RM-2026-0075`: le total ne franchit cette frontiere qu'avec sa provenance.
    # Un substitut n'est pas efface - il change de libelle. Voir `_montant_source`.
    total_charges_lu = _accounting_total_charges(
        expense_statement,
        ledger,
        invoices,
        matches,
    ...
    summary["total_charges_source"] = total_charges_lu.source
    summary["total_charges_lu_sur"] = total_charges_lu.lu_sur if total_charges_lu.valeur is not None else ""
    if total_charges_lu.repond:
        summary["total_charges"] = round(total_charges_lu.valeur, 2)
        if summary.get("total_charges_label") in {None, "", "A calculer", "Montant non charge", NON_CALCULABLE}:
            summary["total_charges_label"] = _amount_label(total_charges_lu.valeur)
    else:
        # La source demandee n'a rien rendu. Aucun nombre ne prend le libelle
        # `Total charges`, y compris un nombre deja pose par une passe anterieure.
        summary.pop("total_charges", None)
        summary["total_charges_label"] = NON_CALCULABLE
        if total_charges_lu.substitue:
            summary["total_charges_substitut"] = round(total_charges_lu.valeur, 2)
            summary["total_charges_substitut_label"] = total_charges_lu.mention_substitut(_amount_label)
...
def _enrichit_depuis_le_screening(table: DataTable, screening: DataTable) -> DataTable:
    """Donne au type et a la date de chaque piece le chemin jusqu'a son libelle.

    `file_biffage.csv` ne porte ni `document_type` ni `suspected_date` - ses
    colonnes s'arretent au college et aux transformations requises. Sans eux, le
    libelle public se degrade en `Document` pour toute la file, et deux pieces
    deviennent indistinguables. Le registre de screening, lui, les porte: on les
    rapatrie par `doc_id` avant de calculer le libelle.
    """

    par_doc = {row.get("doc_id", ""): row for row in screening.rows if row.get("doc_id")}
    if not par_doc:
        return table
    enrichies = []
    for row in table.rows:
        source = par_doc.get(row.get("doc_id", ""))
        if source:
            for champ in ("document_type", "suspected_date"):
                if not row.get(champ) and source.get(champ):
                    row = {**row, champ: source[champ]}
        enrichies.append(row)
    return DataTable(path=table.path, fields=table.fields, rows=enrichies)


def _privacy_model(instance: InstanceConfig) -> dict[str, object]:
    paths = _privacy_paths(instance)
    screening_brut = _read_table(paths["screening"])
    screening = _safe_privacy_rows(screening_brut)
    redactions = _read_table(paths["redactions"])
    queue = _safe_privacy_rows(_enrichit_depuis_le_screening(_read_table(paths["queue"]), screening_brut))
    review_rows = [
        row
