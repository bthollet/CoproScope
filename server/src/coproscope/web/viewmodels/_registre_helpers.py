...
def _document_label(doc_id: str, row: dict[str, str] | None = None, fallback: str = "Piece locale") -> str:
    """Libelle d'une piece pour `pieces.html`, `piece_detail.html`, `actions.html`.

    `file_name` figurait ici entre `display_name` et `title`: un nom de fichier
    ecrit par le syndic devenait donc le libelle de la piece sur trois ecrans.
    Il est remplace par le libelle derive du type et de la date.
    """

    source = row or {}
    explicite = source.get("display_name") or source.get("title")
    if explicite:
        return _public_text(explicite, fallback)
    derive = libelle_public_document({**source, "doc_id": source.get("doc_id") or doc_id})
    return _public_text(derive or doc_id, fallback)
