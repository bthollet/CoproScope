...
# Le critere de remontee du classement n'est PAS une liste de statuts douteux.
# Il est le complement de la liste fermee des statuts qui affirment un
# classement etabli, tenue par `core.classement_etabli`. Un statut inconnu
# remonte donc par defaut, au lieu d'etre ignore en silence. La liste de
# modalites qui vivait ici manquait `A_RECLASSER` et `TEXTE_INSUFFISANT` tout
# en devinant deux valeurs que le code ne produit nulle part.
...
def _apply_document_intake_saved_choices(
    ...
    for row in rows:
        ...
        if choice:
            ...
            copy["classification_status"] = TYPE_A_CLASSER if type_doit_remonter_a_l_humain(document_type) else "PROPOSED"
...
def _save_document_intake_register_choice(
    ...
    for row in rows:
        ...
        row["classification_status"] = TYPE_A_CLASSER if type_doit_remonter_a_l_humain(document_type) else "PROPOSED"
...
def _document_intake_rows_from_register(
    ...
    *,
    ...
    for register_row in register_rows:
        ...
        needs_review = (
            doit_remonter_a_l_humain(classification_status)
            or type_doit_remonter_a_l_humain(document_type)
        ...
        rows.append(
            {
                ...
                # Le statut brut voyage tel quel: c'est la couche de
                # qualification qui le traduit en phrase pour l'humain, y compris
                # quand elle ne le connait pas. Ne rien ajouter ici de non lu par
                # la vue - un champ transporte mais jamais affiche ressemble a
...
def register_document_intake_write_routes(
    app,
    instance,
    _require_token,
    _url_with_token,
    ...
    """Les deux ecritures du depot: qualifier un document, le rattacher.

    Sorties de `_app_fragments/part_003.pyfrag`, qui repassait au-dessus de la
    limite de 600 lignes du depot. Elles vivent mieux ici: toutes les aides
    qu'elles appellent - `_document_intake_*`, `_save_document_intake_*` -
    etaient deja dans ce module, et l'appel traversait la frontiere a chaque
    ligne.
    """
