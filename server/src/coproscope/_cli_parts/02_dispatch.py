...
def _dispatch(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.command == "pipeline" and args.pipeline_command == "run":
            rapport = run_pipeline(instance, run, copy_classified=not args.no_copy, docai_mode=args.docai)
            # Le verdict DERIVE de ce que les etapes declarent. La version
            # precedente testait deux cles precises - `coffre_non_declare` et
            # `registre_vide` - donc un passage qui ecrasait 84 cles de
            # resolution, en les nommant une par une, concluait `ok`. Voir
            # `core/_pipeline_verdict`.
            verdict = verdict_du_passage(rapport.get("gouvernance", []))
            rapport["command"] = "pipeline run"
            rapport["status"] = verdict["statut"]
            rapport["verdict"] = verdict
            if verdict["empechements"]:
                # Compatibilite: ce nom etait deja lu par des appelants.
                rapport["etapes_sans_effet"] = [e.split(":", 1)[0] for e in verdict["empechements"]]
            print(json.dumps(rapport, indent=2, ensure_ascii=True, default=str))
            reussi = verdict["statut"] == STATUT_OK
            run.finish("OK" if reussi else "WARN", "pipeline run complete")
