...
from ._client_du_document import designations_declarees, fragments_de_rejet
...
class DocumentExtractionEvidence:
    ...
    layout_json: str = ""
    visual_review: str = ""
    #: Les designations sous lesquelles le CLIENT apparait sur ses propres
    #: factures. Lues dans l'instance, jamais ecrites dans le code: cf.
    #: `_client_du_document`.
    designations_du_client: tuple[str, ...] = ()
...
def _is_bad_supplier_candidate(line: str, fragments: list[str] | None = None) -> bool:
    ...
    if normalized in {"electricite", "local", "local technique"}:
        return True
    if any(fragment in normalized for fragment in (fragments or list(fragments_de_rejet()))):
        return True
...
def _supplier_near_legal_markers(text: str, fragments: list[str]) -> str:
    ...
    for index, line in enumerate(lines):
        ...
        for candidate in reversed(lines[max(0, index - 8) : index]):
            if len(candidate) < 3 or _is_bad_supplier_candidate(candidate, fragments):
                continue
            supplier = _sanitize_supplier_line(candidate)
            if supplier and not _is_bad_supplier_candidate(supplier, fragments):
                return supplier
    return ""
...
def _guess_supplier(text: str, fragments: list[str]) -> str:
    company_hint = ""
    ...
    legal_hint = _supplier_near_legal_markers(text, fragments)
    if legal_hint:
        return legal_hint
...
def extract_generic_invoice_fields(
    text: str,
    file_name: str = "",
    designations_du_client: tuple[str, ...] = (),
) -> InvoiceExtraction:
    fragments = fragments_de_rejet(designations_du_client)
    supplier = _guess_supplier(text, fragments)
...
def extract_generic_invoice_from_evidence(evidence: DocumentExtractionEvidence) -> InvoiceExtraction:
    return extract_generic_invoice_fields(
        evidence.combined_text(),
        file_name=evidence.file_name,
        designations_du_client=tuple(evidence.designations_du_client),
    )
