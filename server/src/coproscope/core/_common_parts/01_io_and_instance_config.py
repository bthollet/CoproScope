...
def _marque_artefact_produit(path: Path) -> None:
    """`RM-2026-0061`: estampiller l'artefact AU MOMENT ou il est ecrit.

    C'est le seul endroit ou *ce fichier vient du produit* est connu de facon
    certaine. Le classement, lui, ne voit qu'un fichier pose quelque part - et
    les deux marques existantes ne repondaient pas: `source_kind` est derive du
    chemin et vaut `raw` pour 1090 documents sur 1090, et `action_log.csv` ne
    connait que les courses journalisees, or les 11 fichiers de travail mal
    classes ont ete produits hors de toute course.

    **L'import est local a dessein.** Ce module est charge tres tot, et une
    dependance de module a module creerait un cycle. **Et la marque ne doit
    jamais faire echouer une ecriture**: un registre d'estampilles est une aide
    au classement, pas une condition de production. D'ou le `except`, qui est
    ici un choix et non un oubli.
    """
    from coproscope.core.estampille_artefact import marque_produit

    try:
        marque_produit(path)
    except OSError:  # pragma: no cover - un disque plein ne perd pas l'ecriture
        return
