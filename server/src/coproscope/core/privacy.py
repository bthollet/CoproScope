...
from ._privacy_couverture import (
    CHAMP as CHAMP_COUVERTURE,
    INDETERMINEE,
    PARTIELLE,
    couverture_lue,
    la_plus_prudente,
...
POLICY_FIELDS = [
    ...
    # Quelle part du document le controle a REELLEMENT lue. Un verdict
    # rendu sur un echantillon et un verdict rendu sur tout ne se
    # distinguaient par aucune colonne avant ce champ.
    CHAMP_COUVERTURE,
]
...
#: Les deux colleges qui interdisent tout traitement par un modele.
COLLEGES_SANS_IA = {"C7_Contentieux_Secret", "C8_Restreint_Critique"}


def _plafond_ia(college: str) -> str:
    """Le plafond de traitement IA d'un objet, d'apres SON college.

    Applique au brut comme au derive. La distinction est le tout: un derive
    caviarde n'a plus de raison de porter le plafond de la piece dont il est
    tire, sinon le caviardage ne sert a rien.
    """

    return "no_ai" if college in COLLEGES_SANS_IA else "local_only"
...
def build_access_policy(row: dict[str, str], text: str, instance: InstanceConfig | None = None) -> dict[str, str]:
    ...
    # **Le troisieme plafond, et il etait le plus discret des trois.**
    # Meme quand l'appelant passe le document entier, le verdict n'en
    # confronte que les 50 000 premiers caracteres aux detecteurs. Le
    # plafond reste - il borne le cout d'un balayage sur 858 pieces -
    # mais il cesse d'etre muet.
    vu = text[:50000]
    couverture = la_plus_prudente(
        row.get(CHAMP_COUVERTURE, "") or INDETERMINEE,
        couverture_lue(len(vu), len(text)),
    )
    haystack = "\n".join(
        [
            path_haystack,
    ...
    ai_processing_ceiling = _plafond_ia(raw)
    # Le derive porte SON plafond, pas celui de sa source. Mesure du 2026-09-07:
    # 478 documents sur 825 etaient `no_ai` parce que leur piece d'origine est
    # en C7 ou C8 - et le restaient une fois caviardes, ce qui interdit
    # exactement l'usage pour lequel le caviardage est produit. Le champ
    # existant n'est pas touche: il decrit le brut, et `docai` s'en sert a bon
    # droit pour refuser de traiter la piece d'origine.
    derivative_ai_ceiling = _plafond_ia(derivative)
    review_required = _review_required(raw, publication_form, confidence, categories)
    # **La consequence, et c'est elle qui repare le defaut mesure.** Un
    # controle qui n'a pas tout lu ne conclut pas: son « rien trouve » ne
    # porte que sur ce qu'il a lu. Mesure du 2026-09-11 sur 64 PDF de plus
    # de dix pages: 19 portaient une categorie que le scan ne voyait pas,
    # dont 12 une categorie CRITIQUE - et quatre etaient classes
    # `nominative` SANS relecture exigee alors qu'ils relevent de
    # l'article 9 du RGPD.
    #
    # L'exigence ne depend PAS de ce qui a ete trouve, et c'est tout son
    # interet: un document dont l'echantillon revele deja des noms peut
    # porter un IBAN trente pages plus loin.
    # **Seule `PARTIELLE` declenche, et ce choix se justifie par un nombre.**
    # `INDETERMINEE` est l'etat des trois autres chemins qui appellent ce
    # verdict sans dire ce qu'ils ont lu - dont l'inventaire, qui classe sur
    # les seules metadonnees AVANT toute extraction et se fait ecraser ensuite.
    # Le faire declencher mettrait les 858 pieces en `A_REVOIR` des le premier
    # passage: un signal qui se leve partout ne designe plus rien, et finit
    # eteint. L'etat reste ecrit au registre, donc visible et denombrable.
    if couverture == PARTIELLE:
    ...
    return {
        ...
        "derivative_ai_ceiling": derivative_ai_ceiling,
