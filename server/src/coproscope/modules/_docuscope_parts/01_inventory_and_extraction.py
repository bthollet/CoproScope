...
from ..core.date_du_document import date_du_document, note_de_date
...
def _extract_plain_text(path: Path) -> tuple[list[str], str, str]:
    text, note = decode_source(path)
    return [text], "1", note
...
def _extract_html(path: Path) -> tuple[list[str], str, str]:
    raw, note = decode_source(path)
    parser = _VisibleTextParser()
    parser.feed(raw)
    return [parser.text()], "1", note
...
def _write_page_text(text_dir: Path, doc_id: str, pages: list[str], suffix: str = "native") -> Path:
    ...
    for index, text in enumerate(pages, start=1):
        lines.append(_marqueur_page.ecrire(index))
        lines.append(text)
        lines.append("")
...
def _process_row(instance, row: dict[str, str]) -> dict[str, str]:
    ...
    try:
        if ext == "pdf":
            ...
            pages, page_count, note_encodage = _extract_plain_text(path)
            extraction_level = "L1_NATIVE_TEXT"
        elif ext in HTML_EXTENSIONS:
            pages, page_count, note_encodage = _extract_html(path)
    ...
    if note_encodage:
        row["notes"] = append_note(row.get("notes", ""), note_encodage)
