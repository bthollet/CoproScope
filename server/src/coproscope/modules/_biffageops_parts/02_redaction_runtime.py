...
def _fusionne_par_document(docs: list[dict[str, str]]) -> tuple[list[dict[str, str]], int]:
    """Une piece se biffe une fois, pas une fois par chemin ou elle est rangee.

    Le registre des documents garde une ligne par CHEMIN: un meme contenu depose
    sous deux noms y occupe deux lignes, avec le meme `doc_id` puisque celui-ci
    est l'empreinte du contenu. La file en heritait - 564 lignes pour 559 pieces
    sur l'instance mesuree le 2026-09-07. C'est le symetrique en aval de
    `RM-2026-0088`.

    Les deux lignes ne sont pas fusionnees en en choisissant une: on prend
    l'UNION de leurs transformations requises. Trancher entre deux lignes qui se
    contredisent sous-protegerait la piece une fois sur deux; l'union ne peut
    que sur-proteger, ce qui se corrige a la revue.
    """

    par_document: dict[str, dict[str, str]] = {}
    ordre: list[str] = []
    fusionnees = 0
    for doc in docs:
        ...
        if not doc_id:
            continue
        connu = par_document.get(doc_id)
        if connu is None:
            par_document[doc_id] = dict(doc)
            ordre.append(doc_id)
            continue
        fusionnees += 1
        transformations = {
            morceau
            for source in (connu.get("required_transformations", ""), doc.get("required_transformations", ""))
            for morceau in source.split(";")
            if morceau
        }
        connu["required_transformations"] = ";".join(sorted(transformations))
        if doc.get("review_required"):
            connu["review_required"] = doc["review_required"]
    return [par_document[doc_id] for doc_id in ordre], fusionnees


def build_redaction_queue(instance: InstanceConfig, run: RunContext) -> dict[str, object]:
    _, docs_bruts = read_csv(instance.register("documents"))
    docs, fusionnees = _fusionne_par_document(docs_bruts)
    ...
    run.log_action("biffage_queue", path, f"rows={len(rows)}; lignes_fusionnees={fusionnees}")
    return {
        "status": "ok",
        "queue": str(path),
        "queued_count": len(rows),
        # Publie, et pas seulement journalise: une deduplication silencieuse est
        # un comptage qui bouge sans que personne sache pourquoi.
