...
# Les deux etats de doute produits par le classement par signature de forme, et
# l'etat qui recueille ce que cette version ne sait pas lire. Ils ne sont pas
# decoratifs: avant qu'ils existent ici, `_normalize_classification_status`
# rabattait tout inconnu sur `PENDING`, puis `_normalize_classification`
# promouvait `PENDING` en `PROPOSED` des qu'un type etait present. Un document
# que le classement avait explicitement mis en doute ressortait donc a l'ecran
# en "propose", coche OK, pret a passer a la suite. Le doute n'etait pas
# seulement perdu: il etait retourne en affirmation.
CLASSIFICATION_DOUBT = "A_RECLASSER"
CLASSIFICATION_NO_TEXT = "TEXTE_INSUFFISANT"
CLASSIFICATION_UNKNOWN = "STATUT_INCONNU"
...
#: Ce que la checklist dit a l'humain pour chacun des doutes, en clair. Les
#: trois se ressemblent a l'ecran si on ne les distingue pas, et ils appellent
#: des gestes differents: relire le document, aller chercher un exemplaire
#: lisible, ou signaler un statut que le produit ne sait pas interpreter.
CLASSIFICATION_DOUBT_MESSAGES = {
    CLASSIFICATION_DOUBT: (
        "Le contenu a ete lu, mais le titre attendu pour ce type n'y figure pas."
        " Confirmer le type a la main, ou en choisir un autre."
    ),
    CLASSIFICATION_NO_TEXT: (
        "Aucun texte exploitable n'a pu etre lu dans cette piece: le type n'est"
        " ni confirme, ni infirme. Verifier l'exemplaire ou relancer l'OCR."
    ),
    CLASSIFICATION_UNKNOWN: (
        "Statut de classement non interprete par cette version. A verifier a la"
        " main avant tout rattachement probatoire."
    ),
}

#: Statuts qui n'affirment aucun classement etabli et exigent une relecture.
#: `CLASSIFICATION_UNKNOWN` en fait partie: ne pas comprendre un statut est une
#: raison de montrer la piece, jamais de la considerer reglee.
CLASSIFICATION_NEEDS_HUMAN = frozenset(
    {
        CLASSIFICATION_PENDING,
        CLASSIFICATION_UNSURE,
        CLASSIFICATION_DOUBT,
...
def _classification_item(intake: DocumentIntakeInput) -> ChecklistItem:
    ...
    if classification.status in CLASSIFICATION_DOUBT_MESSAGES:
        # Un type EST propose, mais le classement a dit pourquoi il en doute.
        # Cette branche passe avant le test du type present: sinon le doute
        # disparaissait derriere un type rempli.
        return ChecklistItem(
            STEP_CLASSIFICATION,
            "Classification: doute signale par le classement",
            CHECK_TODO,
            CLASSIFICATION_DOUBT_MESSAGES[classification.status],
        )
...
def _normalize_classification_status(value: str) -> str:
    ...
    # Un statut que cette version ne connait pas devient `STATUT_INCONNU`, pas
    # `PENDING`. Le rabattre sur `PENDING` le rendait promouvable en `PROPOSED`
    # des qu'un type existait: l'inconnu ressortait alors en proposition reglee.
    return aliases.get(token, token if token in CLASSIFICATION_STATUSES else CLASSIFICATION_UNKNOWN)
