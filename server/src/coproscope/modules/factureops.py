...
from ..extractors.invoices._client_du_document import designations_declarees
...
from ._factures_doublons import anomalies_de_doublon, cle_de_doublon
from ._factures_admission import admettre_facture
...
def _document_evidence(instance: InstanceConfig, row: dict[str, str], path: Path) -> tuple[DocumentExtractionEvidence, str, str]:
    # Le CLIENT d'une facture n'en est pas le FOURNISSEUR. Sa designation
    # se LIT ici, dans l'instance; jusqu'au 2026-09-08 elle etait ecrite en
    # dur dans l'extracteur, pour deux coproprietes rencontrees.
    designations = designations_declarees(instance)
    ...
    if docops_text:
        ...
        return (
            DocumentExtractionEvidence(
                ...
                designations_du_client=designations,
            ),
            "docops_text",
            row_level,
        )
...
def _looks_like_invoice(row: dict[str, str], path: Path | None, text: str, year: int) -> bool:
    """Admet une piece au corpus facture sur sa FORME, pas sur son vocabulaire.

    Cette fonction admettait tout document dont le debut contenait l'annee et
    l'un des mots `facture`, `invoice`, `avoir`, `note d'honoraires`,
    `total ttc`. Un proces-verbal d'assemblee parle de factures; un tableau de
    bord aussi. C'est ainsi que 4 lignes sur 601 en sont venues a porter 94,5
    pour cent du total (`RM-2026-0058`).

    La regle de forme et son fondement sont dans `_factures_admission`. Mesure
    du 2026-09-08 sur 341 pieces reelles d'extranet: la regle de vocabulaire en
    admettait 183, la regle de forme en admet 76 - elle retire 119 admissions
    sans forme de facture et en rattrape 12 que le vocabulaire manquait. La
    concentration des quatre plus gros montants tombe de 94,5 a 34,6 pour cent.
    """
    ...
    if str(year) not in haystack:
        return False
    return admettre_facture(text).admise
...
def _extract_invoice_rows(instance: InstanceConfig, year: int) -> list[dict[str, str]]:
    ...
    # `RM-2026-0135`: les dates vues par cle, pour NOMMER ce qui distingue
    # deux ecritures de meme numero et de meme montant.
    ...
    for row in _candidate_source_rows(instance):
        ...
        # `RM-2026-0135`: la cle, le garde-fou et le discriminant de date
        # vivent dans `_factures_doublons`, avec le raisonnement qui les
        # justifie - notamment pourquoi la date n'entre PAS dans la cle.
        cle = cle_de_doublon(
            extraction.fournisseur, extraction.numero_facture, extraction.ttc)
        seen_keys[cle] += 1
        anomalies.extend(anomalies_de_doublon(
            cle, extraction.date_facture, seen_keys[cle],
            seen_dates.setdefault(cle, [])))
        seen_dates[cle].append(extraction.date_facture)
