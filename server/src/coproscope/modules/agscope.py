...
from ..core.date_du_document import date_du_document
...
def analyze(instance, run: RunContext) -> dict[str, object]:
    """Prepare le registre des assemblees, et DECLARE ce qu'elle n'a pas su lire.

    Cette fonction rendait `None`. Le verdict du passage la comptait donc
    parmi les etapes sans bilan - ce qui, avant le 2026-09-08, revenait a la
    compter comme reussie quoi qu'il arrive.
    """
    ...
    for doc in docs:
        ...
        # Le chemin de rangement N'EST PAS une source de date. Ce site lisait
        # `original_path` en second recours; c'est le meme defaut que celui
        # corrige le 2026-09-08 dans le classement, a un autre endroit - une
        # correction ne couvre que le site qu'elle touche.
        detected_date, _source_date = date_du_document(text, doc.get("file_name", ""))
    ...
    sans_date = [row["doc_id"] for row in rows if not row["detected_date"]]
    sans_resolution = [
        row["doc_id"]
        for row in rows
        if row["document_type"] == "PV_AG" and row["resolution_count"] == "0"
    ]
    motifs: list[str] = []
    if sans_date:
        motifs.append(
            f"{len(sans_date)} documents d'assemblee sans date lisible: "
            "leur assemblee sera identifiee par le document, donc indiscernable d'une autre"
        )
    if sans_resolution:
        motifs.append(
            f"{len(sans_resolution)} proces-verbaux dont aucune ligne de resolution n'est reconnue"
        )
    return {
        "documents_ag": len(rows),
        "constats": len(findings),
        "sans_date_lisible": len(sans_date),
        "motifs_alerte": motifs,
    }
