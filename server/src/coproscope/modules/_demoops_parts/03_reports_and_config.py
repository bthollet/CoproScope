...
def build_demo_instance(
    ...
    *,
    ...
    # Les noms a ne pas laisser sortir sont enfiles explicitement depuis la
    # source: le garde-fou protege toute instance, pas une seule connue d'avance.
    validation = _validate_generated_instance(instance_dir, noms_locaux(source))
    ...
    # La garde validait un arbre AVANT que trois fichiers y soient ecrits -
    # rapport, manifeste, manifeste de publication. Elle declarait donc propre
    # ce qu'elle n'avait pas regarde, et rien ne l'aurait dit. Second passage,
    # sur l'arbre complet: c'est celui-la qui fait foi.
    validation_finale = _validate_generated_instance(instance_dir, noms_locaux(source))
    if validation_finale["decision"] != validation["decision"]:
        demo_instance_payload["publication_validation"] = validation_finale
        _json(instance_dir / "demo_manifest.json", demo_instance_payload)
        _json(instance_dir / "outputs" / "demo" / "manifest_publication.json", demo_instance_payload)
    validation = validation_finale
