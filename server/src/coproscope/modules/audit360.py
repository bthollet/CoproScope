...
def _sanitize_text_for_cloud_audit(instance: InstanceConfig, row: dict[str, str], text: str) -> str:
    ...
    # Le nom de l'expediteur est biffe sans qu'aucun nom d'entite soit connu du
    # code. La garde ne porte que sur le marqueur deja pose: elle est placee
    # hors de la portee du rebroussement (le `\s*` qui suit) pour qu'une espace
    # ne puisse pas la faire echouer, ce qui rend la substitution idempotente.
    sanitized = re.sub(
        r"(?im)^(De\s*:)(?![ \t]*\[BIFFE_)\s*[^\n<]+$",
        r"\1 [BIFFE_PERSON_NAME]",
        sanitized,
