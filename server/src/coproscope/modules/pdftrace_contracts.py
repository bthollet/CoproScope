...
# Bornes hexadecimales: une suite de chiffres enchassee dans un identifiant hexa
# (ex. DOC-3504975596D8) n'est pas un numero de telephone.
_PHONE_RE = re.compile(r"(?<![0-9A-Fa-f])(?:\+?\d[\d .-]{7,}\d)(?![0-9A-Fa-f])")
...
def validate_document_identity(document_ref: str, document_hash: str) -> None:
    if not document_ref or not _DOC_REF_RE.match(document_ref) or _sensitive_ref(document_ref):
...
def _sensitive_ref(document_ref: str) -> bool:
    """Heuristiques PII adaptees aux refs opaques.

    Un identifiant prefixe par des lettres (DOC-197441098683) n'est pas un
    numero de telephone; une ref purement numerique de 9+ chiffres reste
    consideree comme sensible.
    """

    if contains_private_path(document_ref) or _EMAIL_RE.search(document_ref):
        return True
    if _SENSITIVE_MARKER_RE.search(document_ref):
        return True
    digits = re.sub(r"\D", "", document_ref)
    return len(digits) >= 9 and not re.search(r"[A-Za-z]{2,}", document_ref)
