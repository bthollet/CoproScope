...
from ..core._privacy_couverture import (
...
def _tronque(texte: str, max_chars: int) -> tuple[str, str]:
    """Coupe a `max_chars` et DIT si la coupe a mordu.

    Le seul endroit qui coupe, pour que le seul endroit qui sache le dise.
    Avant ce lot, chacun des quatre lecteurs ecrivait son propre `[:max_chars]`
    et jetait l'information au passage.
    """
    return texte[:max_chars], couverture_lue(min(len(texte), max_chars), len(texte))


def _read_text_file(path: Path, max_chars: int) -> tuple[str, str]:
    try:
        return _tronque(path.read_text(encoding="utf-8", errors="ignore"), max_chars)
    except OSError:
        return "", PARTIELLE


def _read_pdf_text(path: Path, max_chars: int) -> tuple[str, str]:
    """**Dix pages au plus, et ce plafond ne bouge pas dans ce lot.**

    Il est deliberement conserve: le remonter a cinquante deplacerait le defaut
    au document de cinquante et une pages au lieu de le traiter. Ce qui change
    est que le lecteur SAIT combien de pages il laisse derriere lui -
    `document.page_count` est sous sa main - et qu'il le dit desormais.
    """
    try:
        ...
        try:
            ...
            lues = min(total_pages, 10)
            parts: list[str] = []
            for index in range(lues):
                parts.append(document.load_page(index).get_text("text"))
                if sum(len(part) for part in parts) >= max_chars:
                    lues = index + 1
                    break
            texte, par_les_caracteres = _tronque("\n".join(parts), max_chars)
            par_les_pages = couverture_lue(lues, total_pages)
            return texte, la_plus_prudente(par_les_pages, par_les_caracteres)
        finally:
            document.close()
...
def _read_docx_text(path: Path, max_chars: int) -> tuple[str, str]:
    parts: list[str] = []
    try:
...
def _read_xlsx_text(path: Path, max_chars: int) -> tuple[str, str]:
    """**Ce lecteur s'arrete en cours de route et ne peut pas savoir ce qui
    reste** - il quitte au milieu des lignes d'une feuille, sans avoir ouvert
    les suivantes. Sa troncature est donc certaine, et sa couverture aussi:
    `PARTIELLE`, sans pourcentage invente.
    """
    try:
            ...
            for worksheet in workbook.worksheets:
                ...
                for row in worksheet.iter_rows(values_only=True):
                    ...
                    if any(values):
                        parts.append(" | ".join(values))
                    if sum(len(part) for part in parts) >= max_chars:
                        return "\n".join(parts)[:max_chars], PARTIELLE
        finally:
            workbook.close()
        return _tronque("\n".join(parts), max_chars)
    except Exception:  # noqa: BLE001
...
def _read_sample(path: Path, max_chars: int) -> tuple[str, str]:
    """Le texte lu, ET ce que cette lecture couvre du document.

    **Le nom disait deja la verite: c'est un ECHANTILLON.** Ce qui manquait
    n'etait pas la connaissance du fait, c'etait sa transmission au verdict.
    """
    ...
    # Un format qu'aucun lecteur ne connait: zero caractere lu sur un document
    # qui existe. Ce n'est pas une lecture complete de rien, c'est une absence
    # totale de lecture - et c'est le cas que `PARTIELLE` existe pour dire.
    return "", PARTIELLE


def _read_existing_text_artifact(instance: InstanceConfig, row: dict[str, str], max_chars: int) -> tuple[str, str]:
