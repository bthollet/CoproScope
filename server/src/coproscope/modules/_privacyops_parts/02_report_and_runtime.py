...
def _write_report(instance: InstanceConfig, screening_rows: list[dict[str, str]]) -> Path:
    ...
    lines = [
        ...
        f"- Documents dont les transformations requises mentionnent le biffage: {to_redact}",
        f"- Documents a revoir: {review}",
        f"- Documents bloques: {blocked}",
        f"- Decisions avec justification requise: {justification_required}",
        "",
        "> Le libelle ci-dessus est volontairement long. `Documents a biffer`",
        "> designait quatre nombres differents selon l'ecran qui le prononcait :",
        "> celui-ci compte les pieces dont les transformations requises",
        "> mentionnent le biffage; la file de biffage compte plus large, en y",
        "> ajoutant la synthese agregee et les metadonnees seules; et le statut",
        "> `diffusable apres biffage` compte plus etroit, la synthese etant",
        "> prioritaire sur le biffage dans la decision recommandee. Trois",
        "> questions, trois nombres, un seul mot : c'est le mot qui etait faux.",
        "",
    ...
    *,
    ...
    for path in _iter_files(
        ...
        try:
            ...
            continue
        text, couverture = _read_sample(path, max_text_chars)
        if not text.strip():
            # Le repli n'herite PAS de la couverture du lecteur qui a echoue:
            # il lit autre chose - le texte deja extrait - donc il declare la
            # sienne. Reporter la premiere ferait dire a ce verdict qu'il
            # repose sur une lecture qui n'a rien ramene.
            text, couverture = _read_existing_text_artifact(instance, row, max_text_chars)
        # La couverture voyage par la LIGNE, pas par un argument de plus:
        # `apply_access_policy` est appele depuis quatre modules, et trois
        # d'entre eux ne savent rien de ce qu'ils ont lu. Un champ optionnel
        # les laisse dire `INDETERMINEE` sans que leur signature change.
        row[CHAMP_COUVERTURE] = couverture
