...
def _liens_vers_non_servis(html: str) -> list[str]:
    liens = (lien_interne(href) for href in _html_hrefs(html))
...
class AtelierPieceUiTests(unittest.TestCase):
    ...
    def test_missing_pieces_view_renders_actionable_novice_cards(self) -> None:
        ...
        # RM-2026-0183: `Ajouter reponse recue` ouvrait `/depot?intent=proof...` et
        # `Ouvrir action` ouvrait `/actions?selected=`, deux ecrans retires. Aucun
        # lien de la page ne doit plus y mener. Temoin: l'instrument trouve la forme
        # exigee avant.
        self.assertEqual(
            ["/depot?intent=proof&missing_for=PRV-CONTRAT", "/actions?selected=PRV-CONTRAT"],
            _liens_vers_non_servis(
                '<a href="/depot?intent=proof&amp;missing_for=PRV-CONTRAT">a</a>'
                '<a href="/actions?selected=PRV-CONTRAT">b</a>'
            ),
        )
    ...
    def test_missing_pieces_view_falls_back_to_accounting_p1_when_piece_contract_is_empty(self) -> None:
        ...
        # RM-2026-0183: le lien `/depot?intent=proof` visait un ecran retire.
        self.assertEqual([], _liens_vers_non_servis(pieces_text))
