...
class DocumentPrivacyPublicLabelTests(unittest.TestCase):
    ...
    def test_documents_and_privacy_routes_hide_local_paths(self) -> None:
        ...
        # RM-2026-0183: l'ecran `/confidentialite` est supprime (404). Le registre
        # de screening seme ci-dessus reste lu par l'accueil, dans la carte
        # PrivacyOps: la garde se mesure la ou la donnee vit encore.
        accueil_response = client.get("/")
        privacy = unescape(accueil_response.text)
        self.assertEqual(client.get("/confidentialite").status_code, 404)
        self.assertEqual(accueil_response.status_code, 200)
        # Temoin: sans lui, une absence de marqueurs ne prouverait rien si
        # l'accueil cessait de lire le registre de screening. Ce texte n'existe
        # que dans ce registre (`review_next_step`).
        self.assertIn("Produire une synthese sans liste nominative.", privacy)

        # Ce test exigeait l'inverse jusqu'au 2026-09-07: il assurait que le
        # nom de fichier NU restait visible, seuls les chemins etant masques.
        # Ce contrat est renverse. Un nom de fichier est ecrit par le syndic
        # et peut porter un patronyme - c'est ce qui a ete mesure sur
        # l'instance servie en 8788. La piece reste identifiee par son
        # identifiant de document et par un libelle derive du type et de la
        # date.
        self.assertNotIn("source.pdf", documents)
        self.assertIn("DOC-PUBLIC-LABEL", documents)
        self.assertIn("Rapport disponible localement", documents)
        self.assertNotIn("source.pdf", privacy)
