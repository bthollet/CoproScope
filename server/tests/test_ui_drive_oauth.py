...
# Ces marqueurs disent ce qui ne doit JAMAIS etre servi. Ils doivent donc
# designer la valeur qui fuit, pas un mot qui la contient parfois: le marqueur
# nu "scope" se declenchait sur "coproscope_recette_mode", le nom du produit en
# minuscules que la coque injecte dans chaque page. Une garde anti-fuite qui
# accuse sa propre marque ne protege plus rien - elle est juste rouge.
# L'axe est `une valeur de scope OAuth est-elle servie`: on cherche donc le
# parametre de consentement et la cle du fichier de jeton, jamais la sous-chaine.
FORBIDDEN_VISIBLE = (
    "OAuth",
    "scope=",
    "scopes",
...
class UiDriveOAuthTests(unittest.TestCase):
    ...
    def _client(self, app: object | None = None):
        ...
        return TestClient(app or create_app(self.instance, 2025, access_token="drive-mvp-local", rebrancher=frozenset({"synchro_drive"})))
