...
class UiContextBannerTests(unittest.TestCase):
    ...
    def test_partial_is_isolated_and_renders_expected_words(self) -> None:
        ...
        # Sans cible, le bandeau ne pointe vers rien: la page de contexte
        # `/gouvernance` est supprimee (`RM-2026-0183`). Avec une cible, le lien
        # revient - la propriete gardee est "un lien seulement vers une page".
        self.assertNotIn("href=", rendered)
        self.assertNotIn("Voir le contexte", rendered)
        with tempfile.TemporaryDirectory() as temp_dir:
            cible = build_context_banner(self._instance(Path(temp_dir)), next_action_href="/documents")
        avec_cible = unescape(env.get_template("_context_banner.html").render(context_banner=cible))
        self.assertIn('href="/documents"', avec_cible)
        self.assertIn("Voir le contexte", avec_cible)
