...
PRIMARY_UI_ROUTES = [
    ...
    # L'entree 06 de la navigation mene desormais au controle de gouvernance:
    # l'ecran remplace `/ag-contentieux`, il ne s'y ajoute pas.
    ("/controle-gouvernance", "Volet décisions"),
...
class ExpandedUiSmokeRouteTests(unittest.TestCase):
    ...
    def test_primary_ui_routes_are_guarded_and_render_with_local_token(self) -> None:
        ...
        header_response = fresh_client.get("/comptes", headers={"x-coproscope-token": "local-secret"})
        self.assertEqual(header_response.status_code, 200)
        self.assertIn("Controle des comptes", unescape(header_response.text))

        cookie_client = self._client(access_token="local-secret")
        first_response = cookie_client.get("/?token=local-secret")
        self.assertEqual(first_response.status_code, 200)
        follow_up = cookie_client.get("/controle-gouvernance")
        self.assertEqual(follow_up.status_code, 200)
        self.assertIn("Volet décisions", unescape(follow_up.text))
    ...
    def test_retired_screens_answer_not_found(self) -> None:
        """Recette de Brice du 2026-09-13 (`RM-2026-0183`): ces ecrans sont

        supprimes, pas caches. Un retour de l'un d'eux doit se voir ici.
        """
        from coproscope.web.debranchement import RETIRES
