...
def _contenu_de_page(html: str) -> str:
    """Rend le `<main>` de la page, c'est-a-dire ce que CETTE page affirme.

    La coque applicative injecte sa propre navigation et ses propres boutons -
    changer d'instance, basculer en mode test - dans chaque page servie. Une
    garde qui cherche une chaine dans le document entier mesure donc la coque
    autant que la page, et ne peut plus dire ce qu'elle veut dire: que cette
    page-ci n'offre aucune action. L'axe est `la page propose-t-elle une
    action`, et le lieu ou cela se mesure est son contenu propre.
    """
    debut = html.find('<main')
    if debut == -1:
        return html
    ouvre = html.find('>', debut)
    fin = html.find('</main>', ouvre)
    if ouvre == -1 or fin == -1:
        return html
    return html[ouvre + 1:fin]
...
class UiMessagesEntrantsTests(unittest.TestCase):
    ...
    def _client(self, access_token: str | None = None):
        ...
        return TestClient(create_app(self.instance, 2025, access_token=access_token, rebrancher=frozenset({"messages_entrants"})))
    ...
    def test_messages_entrants_route_is_token_guarded_and_novice_readable(self) -> None:
        ...
        self.assertNotIn('href="/messages/entrants?token=local-secret"', response.text)
        ...
        contenu = _contenu_de_page(response.text)
        self.assertNotIn("<button", contenu)
