...
class Audit360ImportTests(unittest.TestCase):
    ...
    def test_sender_redaction_knows_no_sender_name(self) -> None:
        instance = self._synthetic_instance()
        text = (
            "De : CABINET QUELCONQUE\n"
            "De : AUTRE CABINET\n"
            "De : Jean DUPONT\n"
            "Objet : convocation assemblee generale\n"
        )

        sanitized = audit360._sanitize_text_for_cloud_audit(instance, {}, text)

        self.assertEqual(sanitized.count("De : [BIFFE_PERSON_NAME]"), 3)
        self.assertNotIn("CABINET QUELCONQUE", sanitized)
        self.assertNotIn("AUTRE CABINET", sanitized)
        self.assertNotIn("Jean DUPONT", sanitized)

    def test_sender_redaction_is_idempotent(self) -> None:
        instance = self._synthetic_instance()
        text = "De : CABINET QUELCONQUE\nObjet : convocation assemblee generale\n"

        once = audit360._sanitize_text_for_cloud_audit(instance, {}, text)
        twice = audit360._sanitize_text_for_cloud_audit(instance, {}, once)

        self.assertIn("De : [BIFFE_PERSON_NAME]", once)
        self.assertEqual(once, twice)

    def _synthetic_instance(self):
        repo_root = Path(__file__).resolve().parents[2]
        instance_root = self.root / "synthetic_copro"
        if not instance_root.exists():
            shutil.copytree(repo_root / "examples" / "synthetic_copro", instance_root)
        return load_instance(str(instance_root / "instance.yml"), None)
