...
def _hrefs_sous(hrefs: list[str], prefixe: str) -> list[str]:
    """Les liens dont le CHEMIN est `prefixe` ou en dessous, a frontiere de segment."""
    chemins = [(href, re.split(r"[?#]", unquote(href), maxsplit=1)[0]) for href in hrefs]
    return [href for href, chemin in chemins if chemin == prefixe or chemin.startswith(prefixe + "/")]
...
class UiMemoireDetailRouteTests(unittest.TestCase):
    ...
    def _client(self, access_token: str | None = None, rebrancher: frozenset[str] = frozenset()):
        try:
            from fastapi.testclient import TestClient  # type: ignore
        except ImportError:
            self.skipTest("FastAPI test client unavailable")
        return TestClient(create_app(self.instance, 2025, access_token=access_token, rebrancher=rebrancher))
    ...
    def test_memory_event_detail_links_are_token_safe(self) -> None:
        ...
        # RM-2026-0183: `/confidentialite` est supprime; plus aucun lien n'y mene.
        self.assertEqual(_hrefs_sous(hrefs, "/confidentialite"), [])
        self.assertIn(
    ...
    def test_memory_event_detail_accessible_buttons(self) -> None:
        ...
        for expected in [
            ...
            self.assertIn(expected, text)
        # RM-2026-0183: `Preparer version diffusable` visait `/confidentialite`
        # (supprime) et `Ajouter document` vise `/documents/ajouter` (debranche).
        # Leurs libelles survivent en texte, dette comptee par
        # test_aucun_lien_interne_ne_mene_nulle_part: on verifie l'absence du
        # lien, jamais la survivance du libelle.
        hrefs = _html_hrefs(response.text)
        self.assertEqual(_hrefs_sous(hrefs, "/confidentialite"), [])
        self.assertEqual(_hrefs_sous(hrefs, "/documents/ajouter"), [])

    def test_memory_event_detail_add_document_button_when_rebranched(self) -> None:
        # `/documents/ajouter` est debranche, pas supprime (RM-2026-0183): rebranche,
        # le bouton `Ajouter document` doit rester un lien qui garde jeton et evenement.
        response = self._client(access_token="local-secret", rebrancher=frozenset({"ajouter_document"})).get(
            f"/chantiers/{DEMO_EVENT_ID}?token=local-secret"
        )
        ajouter = re.findall(
            r"""<a\b[^>]*href=["'](/documents/ajouter[^"']*)["'][^>]*>\s*Ajouter document\s*</a>""",
        ...
        self.assertEqual(
            ajouter,
            [f"/documents/ajouter?intent=memory&event_id={DEMO_EVENT_ID}&token=local-secret"],
        )
