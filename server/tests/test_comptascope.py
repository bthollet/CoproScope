...
class ComptaScopeTests(unittest.TestCase):
    ...
    def test_un_nom_de_copropriete_contenant_eau_n_est_pas_classe_en_eau(self) -> None:
        """Le libelle porte `eau` DANS un nom propre, et ce n'est pas une charge d'eau.

        La donnee de ce test n'est pas anodine: le nom doit contenir la
        sous-chaine `eau`, sinon le test devient vert **sans plus rien garder**.
        C'est aussi pour cela qu'il est nomme d'apres la propriete gardee et
        non d'apres la copropriete ou le cas a ete rencontre.
        """
        account, family = factureops._account_for_invoice(
            "Abattage d'un pin colle au batiment 22 - Copropriete Beaulieu Pinede",
            "TY SERVICES",
        )
    ...
    def test_reconciliation_uses_local_name_similarity_as_p2_candidate(self) -> None:
        invoices = [
            {
                "doc_id": "DOC-SIM",
                "fournisseur": "A.M MARCANTELLI RENOVATION",
        ...
        expenses = [
            {
                ...
                "supplier_hint": "AM MARCANTELLI",
                "label": "AM MARCANTELLI / coffrage placo",
                "amount": "764.50",
            }
        ]
