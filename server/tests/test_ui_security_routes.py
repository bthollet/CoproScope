...
class UiSecurityRouteTests(unittest.TestCase):
    ...
    def test_token_gate_protects_ui_and_exports_when_token_is_configured(self) -> None:
        # RM-2026-0183: les ecrans debranches restent gardes par le jeton, eprouves rebranches.
        client = self._client(access_token="local-secret", rebrancher=frozenset({"ajouter_document", "messages_entrants"}))
        ...
        # RM-2026-0183: les ecrans retires ne sont plus servis, meme avec le jeton.
        for path in [
            "/actions",
            "/suggestions",
            "/pilotage/activite",
