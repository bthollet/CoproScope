...
class UiComptesRapprochementTests(unittest.TestCase):
    ...
    def test_view_model_replaces_raw_document_labels(self) -> None:
        write_csv(
            ...
            [
                _review_row(
                    ...
                    bloc_copiable=(
                        "Controle - Facture oubar.pdf - # 09_Devis_ventilation_Baillargues.pdf - "
        ...
        self.assertNotIn("Facture oubar", visible)
        self.assertNotIn("oubar.pdf", visible)
    ...
    def test_detail_uses_four_source_matrix_without_bank_overclaim(self) -> None:
        ...
        # L'axe est: *le fragment est importe avec un marqueur de version*, pour qu'une
        # modification atteigne un navigateur qui detient deja la feuille en cache. La
        # valeur exacte du marqueur est une modalite. L'assertion la citait en dur
        # (`?v=20260531-compta-049`), donc toute modification legitime du CSS rendait ce
        # test rouge sans qu'aucune garantie n'ait bouge - et la seule reparation possible
        # etait d'y recopier la nouvelle valeur, ce qui n'apprend rien. Rectifie le
        # 2026-09-07 en corrigeant `.cs-rappro-queue-panel`; meme famille que `RM-2026-0106`.
        marqueur = re.search(r"styles_part_30\.css\?v=([^\"')\s]+)", imports)
        self.assertIsNotNone(
            marqueur,
            "styles_part_30.css est importe sans marqueur de version: une correction de "
            "style n'atteindra pas un navigateur qui detient deja la feuille en cache.",
        )
        self.assertTrue(marqueur.group(1).strip())
