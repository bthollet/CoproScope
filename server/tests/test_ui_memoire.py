...
class MemoireUiTests(unittest.TestCase):
    ...
    def test_chantiers_renders_memoire_passation_sections(self) -> None:
        ...
        # Le nom de fichier ne s'affiche plus depuis le 2026-09-07: il est
        # ecrit par un tiers et peut porter un patronyme. Voir
        # coproscope/core/libelle_public.py. La piece reste identifiee par
        # son identifiant de document.
        self.assertNotIn("note_contentieux.md", text)
        self.assertIn("DOC-CONTENTIEUX", text)
    ...
    def test_chantiers_is_token_gated_and_counter_links_keep_token(self) -> None:
        ...
        # Les compteurs menaient au registre `/actions` et a `/confidentialite`,
        # supprimes par la recette du 2026-09-13 (`RM-2026-0183`): la page ne
        # doit plus offrir de lien vers eux. Tout lien restant garde son jeton.
        from coproscope.web.debranchement import RETIRES, sous

        for href in _html_hrefs(text):
            chemin = unquote(href).split("#", 1)[0].split("?", 1)[0]
            with self.subTest(href=href):
                self.assertFalse(sous(chemin, RETIRES), href)
                if chemin.startswith("/") and not chemin.startswith("/static/"):
                    self.assertIn(f"token={token}", unquote(href))
