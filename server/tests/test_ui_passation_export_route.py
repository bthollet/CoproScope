...
def _hrefs_sous(hrefs: list[str], prefixe: str) -> list[str]:
    ...
    chemins = [(href, re.split(r"[?#]", href, maxsplit=1)[0]) for href in hrefs]
...
class UiPassationExportRouteTests(unittest.TestCase):
    ...
    # RM-2026-0183: test de la route `/actions` supprime avec l'ecran (2026-09-13).
    ...
    def test_passation_blocker_detail_links_are_token_safe_and_preserve_scope(self) -> None:
        ...
        # RM-2026-0183: `/actions` est supprime; le lien `Voir action` ne doit plus
        # mener nulle part, les liens restants gardent jeton et perimetre.
        self.assertEqual(_hrefs_sous(hrefs, "/actions"), [])
    ...
    def test_passation_blocker_detail_unknown_id_has_safe_empty_state(self) -> None:
        ...
        # RM-2026-0183: la sortie `Voir actions ouvertes` visait `/actions`, supprime.
        # Son libelle survit en texte (dette comptee par
        # test_aucun_lien_interne_ne_mene_nulle_part): on garde l'absence du lien,
        # pas la survivance du libelle.
        self.assertEqual(_hrefs_sous(_html_hrefs(response.text), "/actions"), [])
