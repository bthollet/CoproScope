...
class DocOpsCompletenessTests(unittest.TestCase):
    ...
    def test_referential_is_loaded_when_instance_declares_no_matrix_section(self) -> None:
        # Le test de matrice actionnable ecrit lui-meme un CSV de preuves: il
        # emprunte donc toujours la branche `matrices`, et ne pouvait pas voir
        # que l'autre branche lisait une clef racine litterale. Une instance
        # reelle n'a le plus souvent aucune section `matrices`.
        payload = dict(self.instance.payload)
        payload.pop("matrices", None)
        payload.pop("matrices_reference", None)
        self.instance.payload = payload
        self.assertIsNone(self.instance.matrix("proofs"))

        declared = load_structured_file(self.instance.completeness_rules_path())
        expected = len(declared.get("preuves", declared.get("proofs", [])))
        self.assertGreater(expected, 0)

        proofs, referential = docuscope._load_proofs(self.instance)

        self.assertEqual(len(proofs), expected)
        self.assertEqual(referential["trouve"], "oui")

    def test_report_does_not_claim_coverage_when_no_requirement_was_evaluated(self) -> None:
        self._prepare_classified_documents()
        payload = dict(self.instance.payload)
        payload["matrices"] = {"preuves": "./registers/referentiel_vide.csv"}
        self.instance.payload = payload
        proofs_path = self.instance.matrix("proofs")
        assert proofs_path is not None
        write_csv(proofs_path, ["preuve_id", "lot", "libelle_attendu", "type_document"], [])

        run = RunContext(self.instance, "docops completeness referentiel vide")
        report_path = docuscope.missing_docs(self.instance, run)
        report = report_path.read_text(encoding="utf-8")

        self.assertIn("- Pieces attendues: 0", report)
        self.assertNotIn("Toutes les pieces attendues sont couvertes", report)
        self.assertIn("Aucune piece attendue n'a ete evaluee", report)

    def test_unread_documents_are_named_instead_of_counted_as_missing(self) -> None:
        self._prepare_classified_documents()
        documents_path = self.instance.register("documents")
        fields, rows = read_csv(documents_path)
        self.assertTrue(rows)
        illisible = rows[0]
        illisible["lot"] = "A_CLASSER"
        illisible["document_type"] = "A_CLASSER"
        illisible["classification_status"] = "TEXTE_INSUFFISANT"
        write_csv(documents_path, fields, rows)

        run = RunContext(self.instance, "docops completeness documents non lus")
        report_path = docuscope.missing_docs(self.instance, run)
        report = report_path.read_text(encoding="utf-8")
        reports_dir = self.instance.artifact("reports_dir")
        _, unread_rows = read_csv(reports_dir / "documents_non_lus.csv")
        _, matrix_rows = read_csv(reports_dir / "matrice_completude_documentaire.csv")

        unread_ids = {row["doc_id"] for row in unread_rows}
        self.assertIn(illisible["doc_id"], unread_ids)
        self.assertEqual(
            {row["constat"] for row in unread_rows if row["doc_id"] == illisible["doc_id"]},
            {"TYPE_NON_DECIDE"},
        )
        # Un document que l'outil n'a pas su lire est present au dossier. Il est
        # nomme comme tel, et non compte comme une piece absente du dossier.
        self.assertIn("## Documents presents que l'outil n'a pas su lire", report)
        self.assertIn(illisible["file_name"], report)
        self.assertIn(f"- Documents dont la nature n'est pas etablie: {len(unread_rows)}", report)
        absent_matches = [
            row["matched_doc_ids"] for row in matrix_rows if row["status"] == "ABSENT"
        ]
        self.assertEqual([value for value in absent_matches if value], [])

    def test_requirement_without_declared_legal_basis_is_reported_as_conventional(self) -> None:
        self._prepare_classified_documents()
        proofs_path = self.instance.matrix("proofs")
        assert proofs_path is not None
        write_csv(
            proofs_path,
            ["preuve_id", "lot", "libelle_attendu", "type_document", "criticite", "fondement"],
            [
                {
                    "preuve_id": "PRV-SANS-FONDEMENT",
                    "lot": "Assurance",
                    "libelle_attendu": "Attestation assurance en cours",
        ...
        run = RunContext(self.instance, "docops completeness fondement")
        report = docuscope.missing_docs(self.instance, run).read_text(encoding="utf-8")

        self.assertIn("fondement non declare", report)
        self.assertIn("sans fondement declare", report)
