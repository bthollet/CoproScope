...
class UiDriveMvpTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._isoler_les_racines_drive()

    def _isoler_les_racines_drive(self) -> None:
        """**Sans ceci, ces tests lisent le PROFIL REEL du poste.**

        `RM-2026-0174`: `test_drive_real_sync_blocks_public_google_folder_
        without_upload` echouait dans une suite complete puis passait dans une
        autre, sans qu'une ligne du code Drive ait change. La cellule declarait
        la cause non cherchee - *une bissection de la suite complete*.

        **Elle n'exigeait pas de bissection: elle est dans le code.**
        `drive_local_setup._local_base` et `_profile_root` retombent, quand
        leurs variables d'environnement sont absentes, sur
        `%LOCALAPPDATA%/CoproScope/coffres` et `%APPDATA%/CoproScope` - des
        chemins du POSTE, hors de l'arbre de travail, **qui survivent d'une
        execution de la suite a l'autre**. Mesure du 2026-09-11: le profil
        `%APPDATA%/CoproScope` **existe** sur ce poste.

        **Un seul test du fichier posait ces deux variables**, dans un
        `try/finally` qui les RESTAURE ensuite. Les neuf autres lisaient donc
        le profil reel, et l'ordre d'execution decidait de la branche prise -
        d'ou le libelle `Droits Google a verifier` au lieu de `a corriger`, qui
        est un POST arrete plus tot, pas un libelle qui aurait bouge.

        **C'est le meme motif que le venv partage**, que ce depot a deja paye:
        un etat hors de l'arbre de travail rend le vert non reproductible. Les
        racines vivent desormais dans le dossier temporaire du test, pose pour
        TOUS les tests de la classe et retire au nettoyage.
        """
        import os

        from coproscope.modules.drive_local_setup import LOCAL_ROOT_ENV, PROFILE_ENV

        racines = {
            LOCAL_ROOT_ENV: str(Path(self.tempdir.name) / "drive-local"),
            PROFILE_ENV: str(Path(self.tempdir.name) / "drive-profile"),
        }
        self._racines_avant = {nom: os.environ.get(nom) for nom in racines}
        os.environ.update(racines)

    def tearDown(self) -> None:
        import os

        for nom, avant in getattr(self, "_racines_avant", {}).items():
            if avant is None:
                os.environ.pop(nom, None)
            else:
                os.environ[nom] = avant
    ...
    def _client(self):
        ...
        return TestClient(create_app(self.instance, 2025, access_token="drive-mvp-local", rebrancher=frozenset({"synchro_drive"})))
    ...
    def test_drive_route_uses_current_shell_and_drive_css(self) -> None:
        ...
        self.assertIn("styles.css?v=20260904-correctifs-systeme-design", response.text)
        ...
        self.assertIn('href="/documents?token=drive-mvp-local"', response.text)
    ...
    def test_drive_google_folder_can_be_created_after_local_setup_and_google_connection(self) -> None:
        ...
        os.environ["COPROSCOPE_DRIVE_LOCAL_ROOT"] = str(local_root)
        try:
            app = create_app(self.instance, 2025, access_token="drive-mvp-local", rebrancher=frozenset({"synchro_drive"}))
