...
class DriveKeyTransferTests(unittest.TestCase):
    ...
    def test_import_refuses_tampered_or_conflicting_key_code(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            original_code = str(original["secret_code"])
            tampered_code = original_code[:-1] + ("1" if original_code.endswith("0") else "0")
            tampered = drive_key_transfer.import_sync_key_code(local_state_root=state_b, secret_code=tampered_code)
            first_import = drive_key_transfer.import_sync_key_code(local_state_root=state_c, secret_code=original_code)
            ...
            self.assertNotEqual(tampered_code, original_code)
