...
class UiAGContentieuxRouteTests(unittest.TestCase):
    ...
    def test_agcontentieux_route_renders_template_without_configured_token(self) -> None:
        ...
        # La navigation ne porte plus cette route: l'ecran de controle de
        # gouvernance l'a remplacee a l'entree 06, decision du 2026-09-04. La
        # route continue de repondre - elle n'est simplement plus une
        # destination proposee, et la barre reste a 25 entrees.
        self.assertNotIn('href="/ag-contentieux"', response.text)
        self.assertIn('href="/controle-gouvernance"', response.text)
    ...
    def test_agcontentieux_route_uses_existing_token_guard_and_nav_token(self) -> None:
        ...
        self.assertIn("AG, contentieux, passation", response.text)
        self.assertIn("Checklist de passation", response.text)
        # Meme remplacement, avec le jeton: c'est l'entree de gouvernance qui
        # porte desormais le lien de navigation, et elle le porte tokenise.
        self.assertNotIn('href="/ag-contentieux?token=local-secret"', response.text)
        self.assertIn('href="/controle-gouvernance?token=local-secret"', response.text)
        # 06 est devenu le groupe *Controle* (`RM-2026-0183`): la gouvernance en
        # est la sous-entree, nommee pour un lecteur d'ecran par son aria-label.
        self.assertIn("<span>Gouvernance</span>", response.text)
        self.assertIn('aria-label="Controle de gouvernance, du mandat vers l', response.text)
