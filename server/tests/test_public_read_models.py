...
class PublicReadModelTests(unittest.TestCase):
    ...
    # RM-2026-0183: l'ecran `/actions` est retire. Le modele public des actions reste lu par les
    # exports `/exports/actions.*`: les deux gardes ci-dessous y sont reportees. Ce qui ne
    # vivait que dans l'ecran (libelle de diffusion, redirection `/actions/{id}`) ne se garde plus.
    def test_actions_exports_use_public_read_model_without_dashboard_build(self) -> None:
        self._build_public_db()
        try:
            from fastapi.testclient import TestClient  # type: ignore
            from coproscope.web.app import create_app
        ...
        with mock.patch(
            "coproscope.web.app.build_dashboard_model",
            side_effect=AssertionError("actions exports must not build dashboard with public projection"),
        ):
            client = TestClient(create_app(self.instance, 2025))
            responses = {path: client.get(path) for path in ("/exports/actions.csv?priority=P1", "/exports/actions.md?priority=P1")}
    ...
    def test_actions_exports_use_empty_public_model_when_projection_is_absent(self) -> None:
        try:
            from fastapi.testclient import TestClient  # type: ignore
            from coproscope.web.app import create_app
        except ImportError:
        ...
        with mock.patch(
            "coproscope.web.app.build_dashboard_model",
            side_effect=AssertionError("actions exports must not fall back to dashboard when vault is configured"),
        ):
            client = TestClient(create_app(self.instance, 2025))
            response = client.get("/exports/actions.md?priority=P1")

        self.assertEqual(response.status_code, 200)
        self.assertIn("- Actions exportees: 0", response.text)
