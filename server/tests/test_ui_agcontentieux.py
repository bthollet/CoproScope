...
class AGContentieuxUiTests(unittest.TestCase):
    ...
    def test_model_surfaces_ag_contentieux_passation_work_items(self) -> None:
        ...
        # **Le temoin change, l'exigence ne change pas.** Ce test verifiait que la
        # piece de contentieux est bien visible, et s'en assurait en cherchant son
        # NOM DE FICHIER - c'est-a-dire exactement ce que la page ne doit plus
        # rendre depuis `RM-2026-0127`. La piece se retrouve desormais par son
        # identifiant public, qui ne porte aucun patronyme.
        self.assertIn("DOC-CONTENTIEUX", joined)
        self.assertNotIn("note_contentieux_factuelle.md", joined,
                         "le nom de fichier brut ne doit plus atteindre l'ecran")
    ...
    def test_template_renders_seeded_model_with_novice_copy(self) -> None:
        ...
        # Meme substitution de temoin que plus haut: la piece reste retrouvable,
        # par son identifiant public et non par son nom de fichier.
        self.assertIn("DOC-CONTENTIEUX", body)
        self.assertNotIn("note_contentieux_factuelle.md", body,
                         "le nom de fichier brut ne doit plus atteindre la page")
