...
> **STATUT AU 2026-09-02: DOCUMENT HISTORIQUE.** Les outils qu'il decrit,
> `tools/orchestration_watchdog.py` et `tools/orchestration_supervisor.py`, ont
> ete retires du depot. Ils dependaient de mecanismes propres a Codex - les
> automations `.toml` sous `$CODEX_HOME`, la heartbeat `FREQ=MINUTELY;INTERVAL=5`
> et l'arret de processus Codex - qui n'ont pas d'equivalent en session Claude
> Code. La moitie generique, la lecture du registre de presence, est reprise par
> `tools/presence_lint.py`. La vivacite reelle des conversations s'obtient
> desormais cote agent. Voir
> [`protocole_agents_claude_code.md`](./protocole_agents_claude_code.md).
...
- utiliser `examples/synthetic_copro` pour une preuve partageable, ou
  `tilleul_pseudo_test` seulement si le scenario local l'exige;
