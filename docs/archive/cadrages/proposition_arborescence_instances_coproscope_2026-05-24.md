...
Revision: voie temporaire apres challenge de l'inbox reconstruction Tilleuls (pseudo)
...
Le survol de l'inbox reconstruction Tilleuls (pseudo) confirme ce besoin. Elle contient
...
L'inbox Tilleuls (pseudo) montre le probleme d'une arborescence physique:
...
- `02 - A verifier avant partage`: file de securite visible. Le survol
  Tilleuls (pseudo) montre que c'est indispensable: une grande partie des documents
...
## Impacts du challenge inbox Tilleuls (pseudo)
...
7. Tester sur une instance smoke, puis seulement sur `tilleul_pseudo_test`.
