...
```text
http://127.0.0.1:8766/?token=tilleul_pseudo-test-local
```
...
- Faire la recette locale par defaut sur `C:\Users\brice\CoproScope\instances\tilleul_pseudo_test`; garder `examples\synthetic_copro` pour les tests publics/CI et demonstrations partageables.
