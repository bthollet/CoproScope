...
```powershell
.\server\.venv\Scripts\python.exe -m coproscope.cli ui open-test --instance-root "$env:USERPROFILE\CoproScope\instances\tilleul_pseudo_test" --year 2025 --port 8766 --token tilleul_pseudo-test-local
```
