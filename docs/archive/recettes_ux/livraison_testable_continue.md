...
`C:\Users\brice\CoproScope\instances\tilleul_pseudo_test`.
...
La cible de recette live locale par defaut est `C:\Users\brice\CoproScope\instances\tilleul_pseudo_test`. `examples/synthetic_copro` reste la cible publique/CI.
...
- `/api/model`: resume declare du modele - branches, tables et comptes -,
  protegee par token. Depuis `RM-2026-0130` (2026-09-09) elle ne rend plus le
  modele verbatim: aucune valeur lue dans un registre n'en sort.
