...
- `RM-2026-0017` est le P0 operationnel courant: reconstruction progressive
  Tilleuls (pseudo), simulation comparee au reel, pieces primaires filtrees et
  pipeline local.
...
Tilleuls (pseudo) comme banc de preuve reel pour mesurer performance, robustesse et
qualite utilisateur.
...
| 2 | Couloir `piece -> detail -> relance -> depot` | GO technique local-first | Routes reelles, token conserve, relance non envoyee, depot contextualise, labels novice visibles, navigateur desktop/mobile OK sur fixture. | Rejouer sur instance Tilleuls (pseudo) reconstruite, avec temps cible et donnees non synthetiques. |
...
| Reconstruction Tilleuls (pseudo) (`RM-2026-0017`) | P0 operationnel actif | C'est le banc de preuve reel/test, pas un second produit. Sa valeur est de demontrer que le systeme reconstruit une base fiable depuis les pieces primaires. |
...
### Lot 1 - Recette reelle sur Tilleuls (pseudo)
