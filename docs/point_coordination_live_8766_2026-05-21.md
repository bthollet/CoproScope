...
- L'environnement de test local par defaut des agents devient `C:\Users\brice\CoproScope\instances\tilleul_pseudo_test`.
- Identifiant de l'instance: `tilleul_pseudo-test`.
...
.\server\.venv\Scripts\python.exe -m coproscope.cli ui open-test --instance-root C:\Users\brice\CoproScope\instances\tilleul_pseudo_test --year 2025 --host 127.0.0.1 --port 8766 --token tilleul_pseudo-test-local
```

### Signal aux agents

- Lire `AGENTS.md` et `docs/orchestration_agents.md` avant toute nouvelle recette.
- Ne pas commiter la base `tilleul_pseudo_test` ni ses dossiers `raw`, `restricted`, logs, exports prives ou tables de correspondance.
- Toute comparaison avec Platanes doit etre annoncee comme test public/CI, pas comme recette locale par defaut.

### Verification

- `doctor` sur `tilleul_pseudo-test`: OK.
...
- Instance cible: `C:\Users\brice\CoproScope\instances\tilleul_pseudo_test`.
...
- Lancer `doctor` et inventaire des sorties derivees sur `tilleul_pseudo_test`.
- Ne pas supprimer les dossiers runtime; si rebuild requis, sauvegarder d'abord les outputs/projections reconstructibles.

### Diagnostic initial

- `doctor --instance-root tilleul_pseudo_test`: OK, chemins et dependances OK.
...
- Si GO: sauvegarde complete de `tilleul_pseudo_test`, puis `pipeline run --no-copy --docai off`, `accounting reconstruct --year 2025`, puis `decisions build` et `incidents build` seulement si leurs registres doivent etre regeneres.

## Point 12:52 CET - Rebuild cible Tilleuls (pseudo) execute

### A tester maintenant

- Site Tilleuls (pseudo) local: `http://127.0.0.1:8772/comptes?token=tilleul_pseudo-test-local`.
...
- `CONV-2026-0063` a execute le runbook sur `C:\Users\brice\CoproScope\instances\tilleul_pseudo_test`.
- Preflight: `doctor` OK, `vault status/verify` OK.
- Sauvegardes verifiees:
  - `C:\Users\brice\CoproScope\dev\instance_support\sauvegardes\tilleul_pseudo_test\20260522-122035` avant pipeline, 2188 fichiers, 0 ecart;
  - `C:\Users\brice\CoproScope\dev\instance_support\sauvegardes\tilleul_pseudo_test\20260522-122854-post-pipeline` apres pipeline, 2195 fichiers, 0 ecart;
  - `C:\Users\brice\CoproScope\dev\instance_support\sauvegardes\tilleul_pseudo_test\20260522-123739-post-accounting` apres compta, 2210 fichiers, 0 ecart.
- Commandes lancees:
...
- Comptes UI: `8772/comptes` 200, marqueur Tilleuls (pseudo) present.
- Actions UI: `8772/actions` 200 apres timeout etendu, marqueur Tilleuls (pseudo) present.
- Pieces UI: `8772/pieces?proof=missing` 200, marqueur Tilleuls (pseudo) present.
- Tests serveur:
...
- Basculer le navigateur de `8771` vers `8772` pour voir la vraie instance tilleul_pseudo.
- Diagnostiquer la lenteur passation/export sur donnees Tilleuls (pseudo) avant de declarer un GO produit complet.
...
- Rejouer sur l'instance cible `8766`/tilleul_pseudo avec un identifiant piece reel de cette instance: le test novice a signale que l'id synthetique `UX-PIECE-COMP-C2B3F479` y renvoie `404`.
...
- GO utilisateur Tilleuls (pseudo) encore conditionnel: rejouer sur `8766` avec un identifiant piece existant dans l'instance cible.
...
## Point 13:17 CET - Perf passation Tilleuls (pseudo)
...
- Rejouer `http://127.0.0.1:8772/actions?token=tilleul_pseudo-test-local` puis `http://127.0.0.1:8772/exports/passation?token=tilleul_pseudo-test-local`.
...
- GO utilisateur Tilleuls (pseudo) encore conditionnel: recharger le live `8772`, finir la recette navigateur, puis seulement profiler plus profond si le dashboard unique reste trop lent.
...
- Apres patch sur `tilleul_pseudo_test` via TestClient: `/actions` 200 en `17.2s`; `/exports/passation` 200 en `22.4s`.
...
- Serveur Tilleuls (pseudo) frais laisse disponible: `http://127.0.0.1:8785/?token=tilleul_pseudo-test-local`.
...
- Verdict utilisateur Tilleuls (pseudo): `NO-GO` tant que le couloir piece -> relance -> depot ne passe pas sans timeout.
...
- QA: l'auto-decouverte est necessaire et prudente; Tilleuls (pseudo) expose des liens detail reels, mais piece/relance depassent les timeouts de recette et depot reste trop lent.
...
- Tilleuls (pseudo) `8785` avec `COPROSCOPE_LIVE_PIECE_ID=auto` et `COPROSCOPE_LIVE_TIMEOUT=35`: echec utile, pas 404; timeout sur relance contextualisee et lenteur constatee sur les routes du couloir.

### Prochain mouvement

- Profiler la construction de modele pour les routes piece/relance sur `tilleul_pseudo_test`.
- Rejouer ensuite le gate auto avec un timeout de recette normalise, puis seulement produire la preuve navigateur tilleul_pseudo.

## Point 14:44 CET - Perf piece relance depot Tilleuls (pseudo)

### A tester maintenant

- Serveur synthetique partageable relance: `http://127.0.0.1:8784/?token=local-secret`, live contract `6 OK`.
- Serveur Tilleuls (pseudo) frais laisse disponible: `http://127.0.0.1:8786/?token=tilleul_pseudo-test-local`.
- Verdict utilisateur Tilleuls (pseudo): toujours `NO-GO` tant que le gate auto ne passe pas avec `COPROSCOPE_LIVE_TIMEOUT=8` et tant que chaque ecran reste au-dessus de 5s.
...
- Tilleuls (pseudo) `8786`, gate auto avec `COPROSCOPE_LIVE_TIMEOUT=35`: `1 OK`.
- Tilleuls (pseudo) `8786`, gate auto avec `COPROSCOPE_LIVE_TIMEOUT=8`: echec attendu par timeout sur `/pieces?proof=missing`, donc NO-GO novice maintenu.
...
## Point 15:42 CET - Perf viewmodel Tilleuls (pseudo)

### A tester maintenant

- Serveur Tilleuls (pseudo) frais laisse disponible: `http://127.0.0.1:8787/?token=tilleul_pseudo-test-local`.
...
- Avant optimisation profonde: routes Tilleuls (pseudo) piece/relance/depot environ `16.0s` / `18.3s` / `17.1s` apres suppression des reconstructions redondantes.
- Apres patch viewmodel, mesures isolees TestClient Tilleuls (pseudo):
...
- Tilleuls (pseudo) `8787`, gate cible piece -> relance -> depot avec `COPROSCOPE_LIVE_TIMEOUT=8`: `1 OK`.
- `git diff --check` sur fichiers touches: OK.
...
- Produire ensuite la recette navigateur Tilleuls (pseudo) multi-viewport seulement si le gate complet est vert et si les ecrans critiques restent proches de 3-5s.
