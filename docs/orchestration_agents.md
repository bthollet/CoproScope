...
- Utiliser `C:\Users\brice\CoproScope\instances\tilleul_pseudo_test` comme environnement de test local par defaut.
...
La reference canonique est la section `Ports locaux` de `CLAUDE.md`. Un port
...
- La recette live locale par defaut se fait sur `C:\Users\brice\CoproScope\instances\tilleul_pseudo_test`.
