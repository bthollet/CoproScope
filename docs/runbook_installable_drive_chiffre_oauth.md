...
### Donner l'acces du coffre a un second poste

Un coffre chiffre ne se lit qu'avec sa cle. Un second poste qui recoit les
paquets sans la cle voit des fichiers illisibles, ce qui est le comportement
voulu. Ces deux commandes transmettent la cle **sans jamais l'afficher**.

- commande `coprocs drive key-export`:

```powershell
.\.venv\Scripts\python.exe -m coproscope.cli drive key-export --local-state-root <etat_local> --output-file <fichier_temporaire>
```

  Ecrit le code d'acces dans le fichier indique. La sortie JSON ne contient
  ni le code, ni le chemin du fichier: elle dit seulement que l'ecriture a eu
  lieu, avec l'empreinte de la cle pour comparaison. Transmettre ce fichier par
  un canal separe du coffre, puis le supprimer.

- commande `coprocs drive key-import`:

```powershell
.\.venv\Scripts\python.exe -m coproscope.cli drive key-import --local-state-root <etat_local> --code-file <fichier_recu>
```

  Installe la cle sur ce poste. Refuse un code altere, et refuse aussi un code
  different quand le poste porte deja une autre cle: dans les deux cas la
  reponse est `blocked` avec un motif lisible, jamais un ecrasement silencieux.

**Ce que ces commandes ne font pas**, et qu'il ne faut pas leur demander: elles
ne transportent rien vers Drive, ne publient rien, et n'affichent aucun chemin
local. La cle ne quitte le poste que dans le fichier temporaire que vous nommez.

**Historique, parce qu'il explique une absence.** Ces deux verbes ont existe,
puis ont disparu du CLI lors d'une integration: le module qui les implemente
est reste complet, mais plus aucun chemin ne permettait de l'atteindre, et
seule une garde de confidentialite s'en est plainte. Retablis le 2026-09-07
sous `RM-2026-0100`.
