...
#: Le nom du dossier d'instance attendu vit HORS du depot, pour la meme
#: raison que les termes interdits de `verifier_avant_push`: ecrire le nom
#: reel d'une copropriete dans le fichier cense proteger ce nom serait la
#: premiere fuite (`RM-2026-0137`).
#:
#: **Sans configuration, l'outil REFUSE au lieu de laisser passer.** Une
#: valeur par defaut ferait qu'un poste mal configure validerait le premier
#: dossier venu, ce qui est exactement le contraire de ce garde-fou.
VARIABLE_INSTANCE = "COPROSCOPE_INSTANCE_RECONSTRUCTION"
...
def instance_attendue() -> str:
    """Le nom du dossier d'instance attendu, ou un refus explicite.

    Trois etats et non deux, comme le controle de confidentialite: trouve,
    absent, et **pas configure**. Le troisieme n'est jamais un feu vert.
    """
    nom = os.environ.get(VARIABLE_INSTANCE, "").strip()
    if not nom:
        raise ProtocolError(
            "Instance de reconstruction non configuree: pose "
            f"{VARIABLE_INSTANCE} sur le nom du dossier attendu. "
            "Ce nom ne vit pas dans le depot."
        )
    return nom
...
def validate_instance(path: str | Path) -> Path:
    """Seul le dossier d'instance declare est accepte.

    **Une verification par nom propre a ete retiree ici le 2026-09-10, et elle
    ne gardait rien.** Elle refusait explicitement une ancienne instance nommee
    en clair dans le code. Or la condition generale qui la suit refuse deja
    TOUT dossier dont le parent n'est pas l'instance attendue - donc cette
    ancienne instance etait refusee deux fois, et la premiere ne changeait que
    le libelle du message. **Ce qui disparait est un message; ce qui reste est
    le refus.** Verifie par mutation avant de retirer la ligne.
    """
    attendue = instance_attendue()
    instance = Path(path).resolve()
    if instance.name.lower() != "instance" or instance.parent.name != attendue:
        raise ProtocolError(f"Instance refusee: attends ...\\{attendue}\\instance.")
...
def new_state(instance: Path, root: Path) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "instance_kind": instance_attendue(),
        "created_at": now_text(),
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    print(f"Instance: {instance_attendue()}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Garde-fou local de reconstruction d'instance.")
